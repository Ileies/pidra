...
- **[INFRA]** One-shot delete of the existing 3428 `mention_count = 0` entities. Script is written and verified (backs the rows up to JSON first, aborts if the two conditions disagree or a `locked` row is in scope, clears `entity_relations` first). Counts confirmed: 3428 = `mention_count = 0` = `last_mentioned IS NULL` = both, out of 3485 total, so the 57 the pipeline has touched are untouched by it. Blocked only on running it
...
- **[BUG]** `prompt_versions` is never read at runtime: `phase5-synthesis.ts:93` imports the prompt constants straight from `src/ai/prompts.ts`, and only the weekly meta-run and the dashboard touch the table. So activating a version on `/prompts` changes nothing, while `CLAUDE.md` presents that approval flow as non-negotiable. Either load the active version in Phase 5 or correct the doc. Also makes `DASHBOARD_PLAN.md` D6 cosmetic until fixed
