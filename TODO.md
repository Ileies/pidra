...
- **[PERF]** The daily Google Tasks snapshot writes one `raw_items` row per open task per day (171 today, roughly 62k a year), because `google.ts:164` puts `runDate` in the dedup key. Nothing reads a past day's snapshot. Either key on the task id plus its `updated` timestamp, or age the snapshots out
