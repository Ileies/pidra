...
- **[DECISION]** Where embeddings are computed is a privacy decision to make explicitly rather than by default. Local Ollama keeps archive content off a cloud API, which matters because the archive contains the personal section - but Ollama was unreliable enough that the Context Builder's extraction path was removed from it. The fallback is an OpenAI embeddings call with `store: false`, and that is a choice to make out loud
