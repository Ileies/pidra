...
- **[FEATURE]** Phases A, B and M per `DASHBOARD_PLAN.md §9` steps 1-3 - design foundation, app shell, mobile. Prerequisites for everything else in the plan
- **[FEATURE]** Phases C, D, E - report reading experience, missing pages, UX polish
- **[FEATURE]** Phase 5 emits `daily_reports.report_json` alongside the markdown (`DASHBOARD_PLAN.md §6 C1`) - the one pipeline change the dashboard plan owns. Unblocked since the `jsonb` fix
