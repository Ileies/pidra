...
- **Not built:** the optional read-only copy of `/runs` and `/sources`. It is the owner's call, and it moved to `TODO.md` (Later) as an open decision.
...
**H4 - honesty where it is still missing.** *Built 2026-09-25; the list below is the brief, and the notes after it say where what shipped differs.*
...
- **The dot needed nothing.** `net.ts` has started at `checking` since H1, and only a request or a probe moves it. The label now names every part that applies ("Offline · 2 not saved"), so a count no longer hides that the app is offline. It also counts failed writes, which it did not count before.
- **A terminal failure used to look saved.** `drain` moves the intent out of the outbox, so the row lost its "queued" chip while the optimistic effect was still in the mirror; the next full pull then took the effect out and, for a create, the row with it. `FailedWrite.svelte` now sits on the note or rule it belongs to. A failed create, or a failed delete whose row is gone, is listed on its own at the top of `/notes` or `/rules`. A rating shows "Not saved", and tapping again replaces the failed rating (`outbox.rate` clears it). Retry re-applies the effect to the mirror before it flushes, because a pull has usually removed it. Discard forces a pull so the row returns to the server's state. `intentIsFor()` in `intents.ts` is now the one place that matches an intent to its row.
- **One line in `Page.svelte`**, not one per page: `SyncAge.svelte` renders on every route in `MIRRORED_ROUTES`, so a new mirrored page cannot leave it out. Past 24 h it becomes the warning the report page used to show on its own, and that banner is gone. The age ticks every 30 s while the page is visible. Before this, "5 min" stayed "5 min" on a page left open.
- **The worker's pull that changed nothing was never announced.** A `304` in the worker (push, periodic sync) moved `lastSyncedAt` in IndexedDB while the open page kept showing the old age. `announce()` now also posts when a pull reached the server.
- **The loading bar shows for server loads, not for first launch.** First launch never used the bar: since H2 no load waits for the network, and `FirstSync` is the screen for that wait. The pages that do make the reader wait are the online-only ones, whose `__data.json` is a real round trip. The bar now shows for any navigation whose target is not in `MIRRORED_ROUTES`. Mirrored pages render from IndexedDB, and on them the bar only flashed.

*Measured* against a local production build in headless Chrome at 390×844. Failed intents were written straight into IndexedDB, so no test write reached the database. On a cold start `/api/health` answered in 53 ms, before the dot first painted at 94 ms, so its first label was "Online". With every API response held back 1.5 s, it read "Checking" at 98 ms and "Online" at 1553 ms. With the API blackholed it read "Checking", then "Offline · 3 not saved". A failed note edit, rule edit and rating each showed on their own row, and a failed create showed above the list. Discard removed it. Client navigation to `/notes` drew no loading bar, and to `/runs` it did. A mirror aged 30 h showed "Offline copy, synced 30 h ago" with "Reconnect to the VPN" while blackholed. Still open for H4's claim: the phone (H5).
