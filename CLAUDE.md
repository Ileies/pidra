...
- **No vector stores yet.** All retrieval today is explicit keyword/entity lookup against Postgres, and nothing in the current phases may add an embedding path. Revised 2026-09-10: a vector store *is* planned, but for the far future and as its own project (design in `DASHBOARD_PLAN.md §10`), not as an incremental addition to a pipeline phase. Until that project starts, treat this as a hard no.
