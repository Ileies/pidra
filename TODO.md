...
- **[FEATURE]** Move the news desks from OpenAI's `web_search` to the Brave Search API (decided 2026-09-25, `CONTEXT_AND_DECISIONS.md` §9). Not a rename: code has to run the search loop, read the pages, enforce the budgets and the rate limit, and give `validate.ts` the result list it checks against. Then delete `researchJson()`, and fold the second `braveSearch()` in `src/server/index.ts` (the one `/api/deepen` uses, which swallows every error) into `src/search/brave.ts`. Design to be agreed before building
...
- **[INFRA]** Once the desks run on Brave: judge the News section on a week of real mornings, above all for what it missed. Compare it against a general news front page and a local one, and tune the desk prompts with `bun run scripts/news-dry-run.ts --editor` (real calls, nothing stored). Then settle the per-desk search budgets on the real Brave request count, which is the cost driver. Note for dry runs: at midday the window starts at midday the day before, so morning stories from yesterday fall outside it; a real 06:30 run does not have that gap
...
- **[INFRA]** The offline device pass: the one check of offline mode (`CLAUDE.md`, Offline mode) that no suite can make. `dashboard/scripts/blackhole/` proves in headless Chrome that no request hangs and every page reaches a designed state within 4 s, but not an installed iOS or Android PWA, a real VPN, or a real push. On the phone at 390×844, in the three real modes - VPN app on with mobile data off (blackhole), VPN off on WiFi (the `10.200.200.1` blackhole), VPN off with DNS answering the public address (gated): cold start, every page, every allowed write (a note created, edited, deleted and restored, two ratings, a rule created and edited), force-quit, reopen offline and see the queue still there, reconnect, watch it flush, then check the rows in Postgres (`created_by`/`updated_by` = `user`, one `note_revisions` row per note mutation, no duplicates). Two numbers are still unmeasured on the device: a cold start over the VPN under 1 s to first report text, and the 06:30 push putting the briefing into the mirror so it opens with the VPN off
